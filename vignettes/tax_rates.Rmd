...
Tax rates, when applied to a property's Equalized Assessed Value (EAV), determine how much a property owner pays in taxes. The tax rates that appear on a bill are calculated by taking a taxing agency's levy (the amount of money requested by the agency to be collected from tax payers) divided by the total taxable value of property within that agency's jurisdiction, also known as the tax base. 

This means that fluctuations to the tax base, and the levy, impact the tax rate. Let's say that property values have gradually risen over the last several years in a given township. When those properties are up for reassessment, we can anticipate that the tax base will grow, and if the base increases while taxing agencies keep their levies stable, then the tax rate will decrease. But what if taxing agencies increase their levies (as they often do within the limits of PTELL)? Would tax rates still decrease but to a lesser extent? Or would they remain stable?

Enter PTAXSIM, which can help us explore the mechanics of Cook County's property tax rates by simulating tax base growth and levy changes to understand the impact on tax rates. This article  examines the impact of reassessments on tax rates historically, followed by a run through of the method used for estimating future rates based on anticipated changes to the tax base and taxing agency levies.
...
## Gathering North tri PINs and tax codes

Since we're interested in looking at tax rate history for the North tri, we'll first need to gather all PINs that fall within North tri townships. We can query these PINs from the Assessor's Office Open Data PIN Universe, filtering by fields `triad_code` and `year` to select only North tri PINs (triad code 2) and in tax year 2023. 

We can construct a query using Socrata's API selecting only the fields we'll need: `pin`, `township_name`, and `tax_code`.
...
Now that we have all North tri PINs and their respective tax codes for tax year 2023, we can use the PTAXSIM database to find each of their tax rates in all years from 2012 to 2023. 
...
```{r}
# Get the township names for each tax code so we find the tax rate distributions for each North Tri township. In some rare cases in Cook County, a tax code can cover multiple townships, but in the North tri each tax code is unique to a township.
north_tri_tax_codes <- north_tri_pins %>%
  group_by(tax_code) %>%
  summarise(township_name = first(township_name)) %>%
  # Query the PTAXSIM database for tax code tax rates from 2012 to 2023
  inner_join(
    dbGetQuery(
      ptaxsim_db_conn,
      glue_sql("
...
The plot below illustrates the percent change in the median tax rate for each township as compared to the prior tax year's rates. Reassessment years 2016, 2019 and 2022 show a consistent pattern of tax rate decreases. In 2022 the median tax rate across the entire North tri was 9.05%, a 10.57% decrease the median 2021 tax rate of 10.12%.
...
## Gathering PIN exemptions, TIF and agency data
...
Next, we need to estimate the total <em>added</em> EAV of each tax code. This involves applying the estimated increases previously derived from the ratio statistics by major class, as well as taking other factors into consideration, such as whether a PIN is subject to a Senior Freeze exemption (in which case we need to make sure taxable EAV stays at its frozen amount), or if the tax code is within a TIF, in which case any growth to that tax code's base will be diverted to the TIF rather than tax code EAV. The key variable we'll use in our estimation of the future tax base is the `added_tax_code_eav`, which is the difference between the reconstructed base year (2023) EAV and the estimated 2025 EAV. 
...
*Not sure if we want to show distribution w/ median, min, max or just medians w/ diff between medians, two tables show those options*
...
rate_summ <- tax_code_rates %>%
  group_by() %>%
  summarise(`2023 Min Rate (%)` = round(min(tax_code_rate_2023)*100,2),
            `2023 Median Rate (%)` = round(median(tax_code_rate_2023)*100,2),
            `2023 Max Rate (%)` = round(max(tax_code_rate_2023)*100,2),
            `Estimated 2025 Min Rate (%)` = round(min(estimated_tax_code_rate_2025)*100,2),
            `Estimated 2025 Median Rate (%)` = round(median(estimated_tax_code_rate_2025)*100,2),
            `Estimated 2025 Max Rate (%)` = round(max(estimated_tax_code_rate_2025)*100,2))


datatable(rate_summ, rownames = FALSE, options = list(dom = "t"))

rate_summ_2 <- tax_code_rates %>%
  group_by() %>%
  summarise(
    `2023 Median Rate (%)` = round(median(tax_code_rate_2023)*100,2),
    `Estimated 2025 Median Rate (%)` = round(median(estimated_tax_code_rate_2025)*100,2),
    `Estimated Rate Difference` = round((median(estimated_tax_code_rate_2025) - median(tax_code_rate_2023)) * 100, 2),
    `Estimated Rate Difference (%)` = round((median(estimated_tax_code_rate_2025) - median(tax_code_rate_2023)) / median(tax_code_rate_2023) * 100, 2)
  )

datatable(rate_summ_2, rownames = FALSE, options = list(dom = "t"))
```

We can also look at how estimated 2025 tax rates compare to 2023 rates across township. The table below shows the median tax rates by township for 2023 and the estimated tax rate for 2025. Across

```{r, echo = FALSE, results='asis'}
# Summarize median 2023 tax rate and median estimated 2025 tax rate by township

rate_town_summ <- tax_code_rates %>%
  left_join(north_tri_tax_codes %>%
...
datatable(rate_town_summ, rownames = FALSE, options = list(dom = "t"))
```
All tax code tax rates for the North tri can be viewed and exported with the table below.
```{r, echo = FALSE, results='asis'}
datatable(
  tax_code_rates %>%
    mutate(`Tax Code` = tax_code, 
           `Tax Code Rate 2023 (%)` = round(tax_code_rate_2023*100, 2), 
           `Estimated Tax Code Rate 2025 (%)` = round(estimated_tax_code_rate_2025*100, 2)) %>%
