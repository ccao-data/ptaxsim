...
A **tax code** is a 5-digit number that identifies the unique combination of overlapping tax districts, such as city, school, and county districts, for a given area. All PINs in the same tax code pay taxes to the same unique combination of districts, and pay the same tax rate.

```{r}
# Get the township names for each tax code so we find the tax rate distributions
# for each North tri township. 
# In some rare cases in Cook County, a tax code can cover multiple townships,
# but in the North tri each tax code is unique to a township.
...
```{r, echo=FALSE}
est_increases %>%
  mutate(`Estimated Increase (%)` = round(est_increase*100,2)) %>%
  select(`Major Class` = class,
         `Estimated Increase (%)`)%>%
  DT::datatable(
    rownames = FALSE,
    options = list(dom = "t")
...
rate_summ <- tax_code_rates %>%
  group_by() %>%
  summarise(
    `2023 Min Rate (%)` = round(min(tax_code_rate_2023) * 100, 2),
