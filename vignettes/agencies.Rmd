...
Additionally, we'll show how to account for the 2024 changes to the Cook County Clerk's agency data structure which will be necessary for users to account for when conducting a time series analysis of certain taxing agencies.
...
We can query the database table `agency_info` to search for `agency_num` key assigned the the taxing agencies of interest. The table below displays all taxing agencies and TIFs with `CHICAGO` presenet in the `agency_name`.
