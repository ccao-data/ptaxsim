...
The code below queries CPI data, then calculates its percent change since 2006. We will need to add the fields `agency_num` and `agency_name` in preparation of combining it with the `chi_levy` data. 
...
cpi <- DBI::dbGetQuery(
  ...
  filter(year >= 2006) %>%
  select(year, agency_num, agency_name, pct_inc)
```
