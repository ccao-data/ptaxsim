...
We can query the `pin_geometry` table from PTAXSIM's SQLite database or use the use the `lookup_pin10() function in order to map Chicago's parcels by their major class codes.
...
![](images/chi_parcel_class_map.png)
...
We again have two sets of bills: real bills from the `mailed` stage, and counterfactual bills from the 2024 `certified` and `board` stages, *as if no commercial appeals were granted*. We can aggregate and index these bills the same way we did previously.
