...
Here we can see the various taxing bodies around Schaumburg. The agency number we want is `030210000` for the `CITY OF CHICAGO`. With the agency number, we can find all of the *tax codes* that make up the municipality. To do so, we can again query PTAXSIM directly, this time looking in the `tax_code` table:
...
chi_pin10s <- unique(substr(chi_pins$pin, 1, 10))

# The geometry is stored in the database as simple text, so we need to convert
# it to the correct type using sf
chi_pins_geo <- lookup_pin10_geometry(year = 2020, pin10 = chi_pin10s)
chi_pins_geo <- chi_pins_geo %>%
...
chi_pins_geo <- st_make_valid(chi_pins_geo)

chi_pins_class_geo <- chi_pins_geo %>%
...
We can then plot the change in the median total tax bill (indexed to the `2023 board` median bill) to see the effect of each stage of appeals. The earlier plot showing assessed values is added for reference.
