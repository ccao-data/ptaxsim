...
Property tax exemptions are savings that lower a homeowner’s property tax bill. They work by multiplying the composite tax rate for the area by a flat amount. For example, in the bill below (PIN = **29-34-135-014-0000**), the local tax rate of 18.103% is multiplied by the Homeowner Exemption flat amount of \$10,000, yielding an exemption amount of \$1,810.30.
...
The PIN we'll use is **29-34-135-014-0000**, the same PIN whose bill is shown above. This is a small, single-family property in Thornton with a very typical exemption situation, just a Homeowner Exemption.
...
```{r}
bills_w_exe <- tax_bill(2006:2020, "29341350140000", simplify = FALSE)
```
...
bills_plot_1 <- bills_w_exe_summ %>%
  ggplot() +
  geom_line(aes(x = Year, y = `Bill Amt.`, linetype = Type)) +
  scale_x_continuous(n.breaks = 9) +
  scale_y_continuous(labels = scales::label_dollar(), limits = c(1000, 5100)) +
...
This PIN's total tax bill has increased since 2006, particularly in the last couple years. Let's see how much it would've increased _without_ its Homeowner Exemption.
...
```{r}
exe_dt <- lookup_pin(2006:2020, "29341350140000") %>%
...
```{r}
exe_dt_2 <- lookup_pin(2006:2020, "29341350140000") %>%
...
Let's look at the overall effect of exemptions in the Cook County township of Thornton, shown in <span style="color:#e41a1c"><strong>red</strong></span> below.

![](../man/figures/exemptions-thornton.png)

First, we can use the PTAXSIM database to get a list of all the unique PINs in Thornton township. We can also create a vector of years we're interested in.
...
Next, we can generate bills for all PINs in Thornton for the past 15 years. These bills will _include_ any exemptions they actually received.
...
Next, we recalculate the base of all taxing districts in Thornton by adding the EAV returned from exemptions to each district's total EAV.
...
```{r}
t_pin_dt_no_exe[, (exe_cols) := 0][, c("exe_total", "tax_code") := NULL]
```

We recalculate all Thornton tax bills _without_ exemptions and with an updated tax base for each district (passed via `agency_dt`).
...
Exemptions in Thornton have significantly increased in both volume and amount (via increased tax rates) in recent years. In 2019, the average residential homeowner saved around $1,500 via exemptions.

Conversely, Thornton's commercial property owners have picked up an increasingly large share of the overall tax burden since 2006. In 2019, the average commercial property paid about $1,500 more than they would have if exemptions did not exist.

### Changing exemptions 

PTAXSIM can also answer hypotheticals about large areas. For example, how would the average residential tax bill in Thornton change if the Senior Exemption increased by $5,000 and the Senior Freeze Exemption was removed?
...
t_pin_dt_new_exe <- t_pin_dt_new_exe[
  ...
  , c("exe_total", "tax_code") := NULL
]
```

We again recalculate all Thornton tax bills with our updated exemptions and with an updated tax base for each district.
...
The net effect of increasing the Senior Exemption while removing the Senior Freeze Exemption is a decrease in the _average_ bill. However, this is complicated by the fact that the Senior Freeze is means-tested, while the Senior Exemption is not. The reality of our hypothetical change is mostly likely an increase in the property tax bills of poorer seniors, even though the average bill decreased.
