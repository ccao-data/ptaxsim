...
Individual property tax bills change the most during a reassessment. However, reassessment typically has only a small impact on the *median* bill (unless reassessment significantly shifts the geographic distribution of the tax base). Showing exactly how bills are affected by reassessment is extremely difficult due to the complexity of Cook County's property tax system.
...
The example PIN we’ll use is **16-26-406-015-0000**. This is a small, single-family property in West Chicago without any exemptions. We can use the `tax_bill()` function to get every bill for this PIN from 2006 to 2020. 

```{r}
p <- tax_bill(2006:2020, "16264060150000")
```
...
p_summ_plot <- p_summ %>% 
  ggplot() +
  geom_vline(
    xintercept = seq(2006, 2018, 3),
    linetype = "dashed",
    alpha = 0.5
  ) +
  geom_line(aes(x = year, y = value, color = type, group = type), size = 1.1) +
  ...
  labs(
    ...
    caption = "Dashed lines represent reassessment years") +
  theme_minimal() +
  theme(
...
This PIN’s assessed value (AV) and total tax bill both decreased from 2006 to 2015 and then subsequently increased. However, the tax bill amount has diverged from the PIN's assessed value and is now near its 2006 high. Why?
...
```{r}
p_tc <- lookup_tax_code(2006:2020, "16264060150000")

# Get historical levies for each district, then set them all to their 2006 level
p_levies <- lookup_agency(2006:2020, p_tc) %>%
...
p_cntr <- tax_bill(2006:2020, "16264060150000", agency_dt = p_levies)
p_cntr_summ <- p_cntr %>%
  select(year, av, final_tax) %>%
  group_by(year) %>%
  summarize(
...
p_cntr_summ_plot <- bind_rows(p_summ, p_cntr_summ) %>%
  ...
  scale_color_brewer(
    ...
    labels = c(
      "Assessed Value",
      "Total Tax Bill",
      "Total Tax Bill (Levies Frozen to 2006)"
    )
  ) +
  labs(
    x = "Year", y = "Indexed to 2006",
...
cw_bills <- tax_bill(2006:2020, cw_pins)
```
...
Assessed values in Ward 22 have significantly diverged from tax bills, with AVs decreasing nearly 50% while tax bills have climbed around 25%.

To find out why, we can again generate counterfactual bills with levies held constant, this time for all of Ward 22.

```{r}
cw_tc <- lookup_tax_code(2006:2020, cw_pins)

cw_levies <- lookup_agency(2006:2020, cw_tc) %>%
  arrange(agency_num, year) %>%
  group_by(agency_num) %>%
  mutate(agency_total_ext = first(agency_total_ext)) %>%
  setDT(key = c("year", "tax_code", "agency_num"))

cw_cntr_bills <- tax_bill(2006:2020, cw_pins, agency_dt = cw_levies)
...
cw_cntr_bills_summ_plot <- bind_rows(cw_bills_summ, cw_cntr_bills_summ) %>%
  ...
  scale_color_brewer(
    ...
    labels = c(
      "Assessed Values",
      "Total Tax Bills",
      "Total Tax Bills (Levies Frozen to 2006)"
    )
  ) +
  labs(
    x = "Year", y = "Median Value, Indexed to 2006",
...
Ultimately, we can say that without levy increases, Ward 22 would have a median tax bill amount roughly 40% lower than it currently does.
...
t_bills_summ <- purrr::map_dfr(2006:2020, function(x) {
  tax_bill(x, t_pins) %>%
    select(pin, year, av, final_tax) %>%
    group_by(pin, year) %>%
    summarize(av = first(av), total_tax = sum(final_tax)) %>%
...
```{r}
t_tc <- lookup_tax_code(2006:2020, t_pins)

t_levies <- lookup_agency(2006:2020, t_tc) %>%
  arrange(agency_num, year) %>%
  group_by(agency_num) %>%
  mutate(agency_total_ext = first(agency_total_ext)) %>%
  setDT(key = c("year", "tax_code", "agency_num"))

t_cntr_bills_summ <- purrr::map_dfr(2006:2020, function(x) {
...
To demonstrate the process of estimating future bills, we can pretend we're in 2017 and try to estimate how the 2018 Chicago reassessment will change bills in Ward 22. We can then compare the *actual* 2018 bills to our estimate.

Let's start by listing our assumptions:

- The 2018 state equalizer will be the same as the 2017 equalizer
- All City triad property AVs will increase by 15% during the 2018 reassessment (3 years of growth + correction from appeals)
- Ward 22 is a hot area so AVs will increase by 25% 
...
We'll set the 2018 equalizer to be the same as the 2017 equalizer.

```{r}
# Pull the equalizer directly from the table included in the
# PTAXSIM database
...
- All City PINs increase 15% from their 2017 AV
- Ward 22 PINs increase 25% from their 2017 AV

```{r}
# Get all township codes in the City triad
cw_town_codes <- c("70", "71", "72", "73", "74", "75", "76", "77")

# Pull ALL PINs (whole county) for 2017 from the DB. We'll manually
...
cw_fut_pins <- cw_fut_pins %>%
  ...
  mutate(
    av = case_when(
      pin %in% cw_pins ~ av_clerk + (av_clerk * 0.25), # Ward up 25%
...
Next, we need to recalculate the tax base of each taxing district using our new estimated 2018 AVs. To do so, we sum the EAV of the PINs in each tax code, then sum the tax code totals to the taxing district (agency) level.
...
# Get the total estimated tax base for each agency for 2018
...
cw_fut_levies <- lookup_agency(
  2014:2017, # Get last 3 years of agency levies
...
Finally, we can use our formatted inputs to estimate what 2018 tax bills would be in Ward 22 given our assumptions. We can then compare the estimated bills to the actual ones to see how close we actually were.

Start by getting all the estimated 2018 bills.

```{r}
cw_fut_bills <- tax_bill(
  year_vec = 2018,
  pin_vec = cw_pins,
  tax_code_vec = lookup_tax_code(2018, cw_pins),
...
cw_fut_bills_plot <- bind_rows(cw_bills_summ, cw_fut_bills_summ) %>%
  ...
  scale_linetype_manual(
    name = "",
    values = c("Estimated" = "dotted", "Actual" = "solid")
  ) +
  scale_x_continuous(n.breaks = 9) +
  scale_color_brewer(
    name = "",
...
The dotted lines show 2018 AVs and tax bills given our assumptions. Both AVs and tax bills to go up significantly, much more than they actually did for Ward 22. Estimated tax bills go up *more* than estimated AVs, indicating that we may have underestimated the AV increase for the rest of the city (or overestimated the growth in Ward 22).
